...
- `master-l8.png` (100-car array): wordmark cap height measures ≈9px at the native 1024px master (≈13px including descender), per the independent visual audit; the L8 journey sprite ships at a 480px canvas (further ≈0.53x downscale), so marks are ≈4–6px and not resolvable as text at any shipped size. The same sub-legible pixels ride inside the shipped L8 sprite (and L9's) — the bundle's text-token gate cannot see raster content, which is why this visual inventory is load-bearing. One cluster met safe-fill criteria and was removed; the remainder is flagged for the Owner-designated offline art route (which can also re-express the array's scale semantics per CTR-042). NOT silently claimed clean.
