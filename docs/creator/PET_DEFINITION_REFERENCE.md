...
`examples/minimal-pet/pet/definition.ts`. This page explains each field and
its constraints. Validation (`pnpm pet:validate`) enforces the machine-
checkable parts and names the exact offending field.

## PetPresentationDefinition

| Field | Type | Notes |
|---|---|---|
| `id` | string | lowercase `[a-z][a-z0-9-]*`, unique, stable; preference key |
...
| `gradePolicy` | object | `showExactLevelNumber`, `showDescription`, `insigniaMode: 'none' \| 'overlay' \| 'wearable'` |
| `gradeLevels` | array | per-level localized descriptions, `id` matching the journey's `levelId`s |
| `behavior` | object | `ambientPool` (ids from `ambient-rules.ts` vocabulary), `stateReactions`, `petting` |
| `speech` | object | category → bilingual lines; categories fixed (`idle`…`ritual`) |
| `license` | object | `license`, `attribution`, `provenance` (pointer to a provenance record) |

## Recipe data

`pose-sprite` requires:
...
- `variantPose`: all ten expression variants (`idle`, `idle-happy`,
  ...
  may share one pose.
- `poses`: PNG+WebP pairs, 320×540 transparent canvas.
- `alphaBounds`: per-pose `[x0, y0, x1, y1]` visible-pixel bounds on the
  canvas — the resident hitbox hugs these (V3 CTR-OVERLAY-003 hit honesty).
- `insignia` (optional): `assets` indexed by journey level and `anchors`
  indexed by pose (`[left%, top%, rotateDeg]`). Required only when
  `gradePolicy.insigniaMode` is `'wearable'` (V8 CTR-040).
