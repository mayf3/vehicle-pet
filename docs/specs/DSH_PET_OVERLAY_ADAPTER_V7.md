...
- one deterministic gesture state model arbitrating click, double-click,
  long-press petting, and drag (§15 CTR-OVERLAY-029/030/032);
- bounded cursor awareness driven by CSS variables and requestAnimationFrame,
  static under reduced motion (CTR-OVERLAY-031);
- one quiet per-character ambient behavior layer on a minutes-level schedule
  with recorded suppression gates (CTR-OVERLAY-033);
- device-local daypart weighting as pure selection weights (CTR-OVERLAY-034);
- a guilt-free welcome-back ritual and once-per-local-day bounded rituals
  through the existing scheduler (CTR-OVERLAY-035/036);
- declarative per-character behavior profiles over the existing presentation
  infrastructure (CTR-OVERLAY-037); small petting/welcome/ritual speech
  category additions to the existing catalogs.
...
# V7 lifelike layer (§15); band values frozen and recorded at implementation
GESTURE_ARBITRATION = DETERMINISTIC_STATE_MODEL (CTR-029)
PETTING = POINTER_LONG_PRESS_ONLY (CTR-030)
PETTING_HOLD_MS = BAND_400_800 (exact value recorded in conformance record)
PETTING_REWARD_SURFACE = NONE
CURSOR_AWARENESS = PROXIMITY_GAZE_CSS_VARS_RAF (CTR-031)
CURSOR_TRACKING_REDUCED_MOTION = STATIC_OR_DISABLED
DRAG_BODY_REACTION = LIFT_TILT_SETTLE (CTR-032; persistence semantics unchanged)
AMBIENT_BEHAVIOR = YES (per-character declarative repertoire, CTR-033)
AMBIENT_GAP_S = RANDOMIZED_MINUTES_LEVEL (band floor 90; exact band recorded)
AMBIENT_LOAD_QUIET_S = >=120
AMBIENT_TYPING_SUPPRESSION_S = >=10
AMBIENT_INTERACTION_COOLDOWN_S = >=120
AMBIENT_NO_BACK_TO_BACK = YES
SECOND_BEHAVIOR_TIMER = PERMITTED_FOR_CTR_033_ONLY
SPEECH_SCHEDULERS = ONE (unchanged)
DAYPART_BUCKETS = MORNING_05_11 / DAYTIME_11_17 / EVENING_17_23 / LATE_NIGHT_23_05 (device-local hour only)
WELCOME_BACK_THRESHOLD_H = BAND_MIN_8 (exact value recorded; NO guilt/streak/duration)
...
## 15. V7 lifelike interaction layer

This whole-authority V7 succeeds V6. It preserves every unchanged V6 decision,
contract, and acceptance and adds one bounded experience layer: direct touch
...
Owner dispatch: Goal「生命感」(EXPERIENCE_PREFLIGHT with PHASE_LOCK, then
authority authoring after the Goal「角色」dependency gate opened). The
read-only census, gap analysis, and interaction/ambient/ritual design inputs
are recorded in `docs/investigations/GOAL_LIFELIKE_PREFLIGHT.md` (OBS/claim
separation there is investigation evidence, not authority). Goal constraints
binding this Spec: `MORE_ALIVE / LESS_SYSTEM`, `RITUAL_WITHOUT_OBLIGATION`,
no neediness systems of any kind, toys and long-term memory stay
FOLLOW_UP_DEBT, one speech scheduler, and no production apply.

### DEC-OVERLAY-019 — Deterministic gesture arbitration replaces ad-hoc guards

- Decision owner: mayf3 (Goal 生命感 frozen direction).
- Decision: one adapter-owned gesture state model owns the resident pointer
  session: `IDLE → PRESSED → {CLICK | DOUBLE_CLICK | PETTING | DRAGGED} →
  (CLICK | PETTING_RELEASE | DROP)`. Movement beyond the recorded drag
  threshold from the press origin routes to DRAGGED at any time; a hold
  passing the petting threshold routes to PETTING only while movement stays
  within its tolerance; release before the petting threshold with no movement
  is CLICK; a second press inside the double-click window after a CLICK is
  DOUBLE_CLICK. Existing mechanics are preserved: the drag threshold, pointer
  capture, ratio persistence, keyboard steps, suppressed-click consumption,
  and the double-click settings menu with Shift+Enter/ContextMenu parity.
- Rejected alternatives: stacking more timeout heuristics on the current
  click/dblclick handlers; touch-library gesture recognizers.
- Reason: five overlapping gestures over one hit surface need one
  deterministic model to stay testable and misfire-free.
- Owner decision remaining: NONE.

### DEC-OVERLAY-020 — Petting is an affectionate long-press, never a button

- Decision owner: mayf3 (Goal 生命感 frozen direction).
- Decision: holding the pet without moving, for a hold within the recorded
  400–800 ms band (exact value frozen by real-feel testing and recorded in
  the implementation conformance record), is petting. During the hold the pet
  presents an affectionate static-family variant with at most one bounded
  melt-class motion and optionally one very short line per episode
  (throttled); release settles back to baseline. Petting never shows scores,
  counters, toasts, progression, or any reward surface, never mutates
  growth, and never fires the click reaction, a bubble stack, or settings.
  A petting hold suppresses the click path entirely (release closes the
  episode without a click).
- Rejected alternatives: affection meters, streaks, hold-to-open-menu,
  haptic/audio feedback.
- Reason: the Goal's core "摸摸" interaction; affection without obligation.
- Owner decision remaining: NONE.

### DEC-OVERLAY-021 — Cursor awareness is a bounded glance, not tracking

- Decision owner: mayf3 (Goal 生命感 frozen direction).
- Decision: while the pointer is near the pet (recorded proximity radius,
  adapter-owned coordinates only), the expression/pose layer may translate
  toward the cursor within ±4 px through two CSS custom properties updated at
  most once per animation frame; no React state updates on pointermove. The
  glance deactivates (eases back to rest) when the pointer leaves the radius
  or rests for a recorded idle window, and is fully disabled during dragging,
  petting, an open menu, hidden documents, working/needs-input states, and
  whenever reduced motion applies (static presentation, `STATIC_OR_DISABLED`).
  The visible figure and its pointer target transform together; grade
  label, footer, and bubble stay still; hit honesty is unchanged.
- Rejected alternatives: continuous head/eye tracking across the viewport,
  per-pixel React state, following the cursor while working.
- Reason: presence without annoyance; the pet notices, it does not chase.
- Owner decision remaining: NONE.

### DEC-OVERLAY-022 — Drag gains body feel; persistence semantics unchanged

- Decision owner: mayf3 (Goal 生命感 frozen direction).
- Decision: a real drag presents a surprised/curious held variant, a bounded
  tilt toward the movement direction (≤4°, figure and hitbox together), and a
  short settle reaction on drop (≤1200 ms, ≤4 px). The existing position
  machinery is untouched: thresholds, clamping, normalized ratio persistence,
  complete-surface menu projection, keyboard movement, and drag suppression
  of click/double-click/petting all behave exactly as V6.
- Rejected alternatives: inertia/throwing physics, position changes on drop
  reaction, reordering drop targets.
- Reason: dragging should feel like lifting a character, not a DIV; the Goal
  forbids changing persistence semantics.
- Owner decision remaining: NONE.

### DEC-OVERLAY-023 — One quiet ambient behavior layer beside the speech scheduler

- Decision owner: mayf3 (Goal 生命感 frozen direction).
- Decision: a per-character declarative ambient repertoire (vehicle:
  look-left, look-right, sensor-check, wheel-blink, small-shuffle; companion:
  stretch, yawn, glance-at-terminal, rest, tidy-cuff, look-around —
  masters/recipes only, no new art) plays on its own slow, quiet schedule:
  randomized minutes-level gaps inside a recorded band (floor ≥90 s; exact
  band frozen by real-feel testing), a ≥120 s load-quiet period, ≥10 s
  payload-ignored typing suppression, a ≥120 s recent-direct-interaction
  cooldown, and no back-to-back repetition of the same action. Attempts
  consumed by suppression/hidden pages are not replayed. Working and
  needs-input states suppress idle ambient actions (state-appropriate
  micro-actions may specialize them); terminal reactions, menu, drag, and
  hidden documents cancel the current action. Ambient actions are one-shot,
  pointer-inert, focus-free, and within the recorded motion bounds. Speech,
  when an ambient action carries an optional short line, still flows through
  the one CTR-019 scheduler — this layer adds a behavior timer, never a
  second speech scheduler. This decision explicitly supersedes the CTR-028
  clause forbidding an additional ambient timer, for this one bounded timer
  only.
- Rejected alternatives: piggybacking ambient actions on the 20–40 s speech
  deadline (too chatty for the Goal's minutes-level requirement), shared
  timer with speech.
- Reason: the Goal's center of gravity — the pet lives when nobody touches
  it, and ANNOYANCE_RISK must stay LOW.
- Owner decision remaining: NONE.

### DEC-OVERLAY-024 — Device-local daypart weighting only

- Decision owner: mayf3 (Goal 生命感 frozen direction).
- Decision: the device-local hour buckets the day as MORNING (05–11),
  DAYTIME (11–17), EVENING (17–23), LATE_NIGHT (23–05). Daypart only
  re-weights ambient action selection, idle expression tendency, and ambient
  speech probability (e.g. sleepy/rest weights rise late night). No calendar,
  location, or message access; no schedule management; no "you should sleep"
  moralizing — at most a rare gentle line inside the ritual category. The
  bucket is a pure input to pure selection functions.
- Rejected alternatives: timezone/location services, scheduled messages,
  strict daypart scripts.
- Reason: behavior varies softly with time; system feel must not appear.
- Owner decision remaining: NONE.

### DEC-OVERLAY-025 — Welcome back without guilt; rituals without obligation

- Decision owner: mayf3 (Goal 生命感 frozen direction).
- Decision: the tolerant browser-local overlay record gains optional
  structured fields for `lastSeenAt` and once-per-local-day ritual markers.
  On mount, an absence of at least the recorded threshold (band ≥8 h, exact
  value frozen at implementation) yields one welcome-back line through the
  existing scheduler's explicit path after the load-quiet period ("回来啦。"
  family). Copy MUST NOT mention absence duration, streaks, loneliness, or
  obligation; absence never penalizes or degrades anything. When a welcome
  fires it takes the first-open-of-day slot; otherwise the Engine
  DailyGreeting stays authoritative (never a double greeting).
  FIRST_COMPLETION_TODAY and LATE_NIGHT each optionally speak once per local
  day through the same explicit path with the same content bounds. Rituals
  are bubble lines only; no system notifications. Unknown/absent/malformed
  fields silently disable the ritual, never block the pet.
- Rejected alternatives: absence decay, streak counters, milestone guilt,
  notification-channel integration.
- Reason: `COMPANION`, not `ANOTHER RESPONSIBILITY`.
- Owner decision remaining: NONE.

### DEC-OVERLAY-026 — Personality is internal behavior weighting, not a setting

- Decision owner: mayf3 (Goal 生命感 frozen direction).
- Decision: each bundled character definition gains a declarative behavior
  profile (ambient weights, reaction-family tendencies, catalog emphasis):
  vehicle energetic and task-oriented (sensor/look/shuffle family, proud
  "跑完啦" completion tone), companion calmer and warmer (stretch/rest/yawn
  family, nodding "搞定" completion tone). The same underlying event renders
  different presentations per character. There is no user-facing personality
  setting and no third character; selection stays pure functions over
  (profile, state, daypart, history, injected random).
- Rejected alternatives: user-configurable personality sliders, per-character
  schedulers, new Engine semantics.
- Reason: two characters must feel different in the small moments the Goal
  names, inside the existing declarative-data pattern of CTR-022.
- Owner decision remaining: NONE.

### CTR-OVERLAY-029 — Deterministic gesture arbitration

The resident MUST route every pointer session through the
`IDLE → PRESSED → {CLICK | DOUBLE_CLICK | PETTING | DRAGGED} → settle`
state model of DEC-OVERLAY-019, with all thresholds and windows recorded as
named constants. The following MUST hold and MUST be covered by a unit
matrix: a click never drags; a drag never clicks, double-clicks, or pets;
a petting hold never emits a click, a double-click, or a settings open; a
double-click sequence emits at most one reaction and never two bubbles;
a drop never triggers an accidental click, reaction, or menu; and the
settled state always returns to the mapped baseline. Keyboard parity is
unchanged (Enter/Space click reaction; Shift+Enter/ContextMenu settings);
the accessible description MUST document long-press petting as
pointer-only. New gesture constants MUST be recorded with their final
values in the implementation conformance record.

### CTR-OVERLAY-030 — Petting interaction

A stationary primary-button hold inside the recorded band (400–800 ms class;
final value recorded) on the pet hit surface MUST present the petting
episode: affectionate static-family presentation from the approved masters,
at most one bounded melt-class motion (≤4 px, ≤4°, duration ≤ the recorded
gesture bound), at most one throttled short petting line per episode, and a
quiet settle on release. During the hold, ambient actions and recurring
speech attempts MUST be suppressed; after release the recent-interaction
cooldown applies. Petting MUST NOT render any score, counter, meter, toast,
reward surface, or progression readout, MUST NOT mutate Engine state (growth
snapshots byte-identical), and MUST NOT open any surface. Reduced motion
presents the static variant without motion. Reactivating input during an
episode replaces it (never queues).

### CTR-OVERLAY-031 — Bounded cursor awareness

When the pointer is within the recorded proximity radius of the resident and
no disabling condition applies (dragging, petting hold, open menu, hidden
document, working/needs-input state, reduced motion), the expression/pose
layer MAY translate toward the cursor by at most ±4 px via CSS custom
properties updated at most once per animation frame with no React state
update on pointermove. The glance MUST ease back to rest after the pointer
leaves the radius or rests for the recorded idle window, MUST keep the grade
label, footer, and bubble static, MUST move the visible figure and its
pointer target together (hit honesty), and under reduced motion MUST be
fully static (`STATIC_OR_DISABLED`) while state readability is unchanged.
No host DOM, no other-plugin surfaces, and no pointer events outside the
adapter's own coordinate space may participate.

### CTR-OVERLAY-032 — Drag body reaction

At drag start (threshold passed) the resident MUST hold a
surprised/curious variant; while dragging it MAY tilt ≤4° toward the
movement direction; on drop it MUST run one settle reaction ≤1200 ms and
≤4 px and return to baseline. The figure and hitbox transform together;
labels/footer/bubble stay still. Position persistence, clamping,
complete-surface layout, keyboard movement, and suppression of
click/double-click/petting during drag are exactly the V6 semantics and
MUST remain byte-compatible (persisted ratios unchanged by the reaction
layer). Reduced motion renders the variant change without motion.

### CTR-OVERLAY-033 — Quiet ambient behavior layer

The adapter MUST implement one ambient behavior timer per resident with:
randomized minutes-level gaps (recorded band, floor ≥90 s), a load-quiet
period ≥120 s, payload-ignored typing suppression ≥10 s, a
recent-direct-interaction cooldown ≥120 s, and no immediate repetition of
the same ambient action. Attempts suppressed by typing, hidden documents,
onboarding, or collapse are consumed without catch-up. Actions render only
from the character's declarative repertoire, one-shot and
pointer-inert/focus-free, within the recorded motion bounds, cancelled by
state change, menu, drag, hidden document, and unmount. Working and
needs-input states MUST suppress idle ambient actions; terminal reactions
always preempt. Any ambient line flows only through the existing CTR-019
scheduler with its gates. Timer/listener disposal follows CTR-012. The
presence MUST remain majority-silent: silence is the baseline, action the
exception.

### CTR-OVERLAY-034 — Daypart weighting

Selection functions MUST accept a device-local hour bucket (MORNING 05–11,
DAYTIME 11–17, EVENING 17–23, LATE_NIGHT 23–05) derived from the local
clock and nothing else, and MUST apply it only as selection weights over
ambient actions, idle expression tendency, and ambient speech probability.
Daypart MUST NOT gate, schedule, or withhold any contracted reaction, and
copy touched by daypart stays inside the ritual category bounds (no
moralizing, no schedules). The weighting MUST be a pure, unit-testable
function of (localHour, character profile, state).

### CTR-OVERLAY-035 — Welcome-back ritual

With a valid persisted `lastSeenAt`, an absence at or beyond the recorded
threshold (band ≥8 h; final value recorded) MUST produce at most one
welcome-back line per mount through the scheduler's explicit path after the
load-quiet period, and MUST NOT produce any guilt phrasing, absence
duration, streak reference, penalty, degradation, or obligation of any
kind. Absent/invalid fields MUST disable the ritual silently. When the
welcome fires, the first-open-of-day greeting slot is considered taken;
otherwise Engine DailyGreeting behavior is unchanged. No other surface
(change of expression allowed) may accompany the welcome.

### CTR-OVERLAY-036 — Bounded daily rituals

FIRST_COMPLETION_TODAY and LATE_NIGHT each MAY produce at most one line per
device-local day, through the scheduler's explicit path, only from the
ritual category, with the CTR-018 content bounds and no moralizing.
Once-per-day semantics MUST survive reload (persisted tolerant day keys)
and MUST NOT accumulate any history beyond the current local day markers.
Rituals are never notifications, never appear during onboarding suppression,
and never fire more than once per local day per kind.

### CTR-OVERLAY-037 — Character behavior profiles

Each bundled character definition MUST carry a declarative behavior profile
consuming the generic presentation infrastructure; Engine sources and
counts-only sources MUST NOT gain character semantics. The two profiles
MUST produce observably different presentations for the same underlying
events (ambient action distributions, reaction-family tendencies, completion
tone) verifiable by pure-function matrices and rendered evidence. Profile
selection MUST NOT add a user-facing personality setting, a third character,
or a second scheduler, and MUST NOT reset cooldowns, deadlines, or history on
character switch beyond the CTR-028 continuity rules.

### Reconciliation with carried contracts

V7 replacements supersede conflicting prior wording, narrowly:

1. CTR-015: the set of permitted overlay-owned motions expands from the
   CTR-028 one-shot gestures to also include the bounded gestures of
   CTR-030/031/032/033. The quiet baseline, OS reduced-motion suppression,
   and no perpetual motion clauses are unchanged and now also bind the new
   surfaces.
2. CTR-018: the category set gains `petting`, `welcome`, and `ritual` with a
   floor of 3 lines each per character per locale; all other floors and the
   content boundary are unchanged. Total catalog growth stays small (no
   38→300 explosion).
3. CTR-019: the trigger-source list gains PETTING, WELCOME_BACK,
   DAYPART_RITUAL/DAILY_RITUAL, and optional AMBIENT_ACTION lines through
   the same single scheduler; existing cadence bounds are unchanged.
4. CTR-021: the gesture surface gains long-press petting and the drag body
   reaction; "dragging MUST cause neither reaction nor menu" now reads as
   "neither the click reaction nor the settings menu" (the CTR-032 body
   presentation is not a click reaction); all other semantics unchanged.
5. CTR-028: the clause "no extra ambient timer" is superseded by
   DEC-OVERLAY-023/CTR-033 for the single ambient behavior timer only.
6. CTR-010: the preference record gains optional tolerant fields for
   `lastSeenAt` and per-local-day ritual markers; absent fields resolve
   safely; every other evolution rule (tolerance, storage-event sync,
   disposal, no Pack duplication) is unchanged.
7. CTR-022: declarative character definitions additionally own behavior
   profiles; no other scope change.

Everything else in V6 — the two-size surface, settings menu, footer,
captions, hit honesty, coexistence, disposal, privacy boundary (CTR-008),
no-panel machine, Engine/progress/token-economy reuse — carries forward
unchanged and remains binding.

### New acceptance items

### ACC-OVERLAY-127 — Gesture arbitration and petting

- Contracts: `CTR-OVERLAY-029`, `CTR-OVERLAY-030`, `CTR-OVERLAY-021`.
- Method: unit matrix over the gesture state model (click/drag separation,
  long-press/double-click separation, double-click single-bubble,
  drop-no-click) with injected clock; browser run in the isolated pinned
  DSH at SMALL and LARGE performing single click, double click, long press
  (inside and beyond the band), pet-and-release, drag-drop; record variant,
  bubble count, and Engine snapshots before/after; reduced-motion run.
- Expected result: the five Goal separation properties hold; petting feels
  affectionate with no score/toast/growth change; settings still opens on
  double-click; keyboard parity intact.
- Failure condition: any misfire (click-drag, drag-click, pet-double-click,
  two bubbles, drop-click), a reward surface, growth mutation, or a
  settings regression.

### ACC-OVERLAY-128 — Cursor awareness bounds and reduced-motion parity

- Contracts: `CTR-OVERLAY-031`, `CTR-OVERLAY-015`.
- Method: drive pointer paths in/out of the proximity radius in the real
  isolated DSH; measure the transform bound, update cadence (rAF), rest
  restore, disabling conditions (drag/petting/menu/hidden/working), hitbox
  co-movement, label/footer stillness; OS reduced-motion run proving the
  static presentation; DOM/React assertion that no per-pointermove render
  occurs.
- Expected result: a small, restoring glance only; static under reduced
  motion; no hitbox drift; no React render churn.
- Failure condition: continuous tracking, bound violation, label/footer
  movement, stale glance, or render-per-move.

### ACC-OVERLAY-129 — Drag body reaction and persistence equivalence

- Contracts: `CTR-OVERLAY-032`, `CTR-OVERLAY-003`, `CTR-OVERLAY-010`.
- Method: drag sequences in the isolated DSH (short, long, clamped at
  viewport edges, over composer region, cancel via pointercancel);
  capture start/tilt/drop evidence; compare persisted preference records
  and restored positions against the V6 controller's expected values;
  reduced-motion run.
- Expected result: lift/tilt/settle present within bounds; persisted
  ratios byte-equal to a no-reaction drag; no accidental click.
- Failure condition: persistence drift, bound violation, reaction on
  suppressed paths, or a hitbox detaching from the figure.

### ACC-OVERLAY-130 — Ambient cadence, repertoire, and suppression

- Contracts: `CTR-OVERLAY-033`, `CTR-OVERLAY-015`, `CTR-OVERLAY-012`.
...
- Expected result: minutes-level quiet cadence with majority silence;
  repertoire and cancellation exact; zero residue after stop.
- Failure condition: sub-minute chatter, catch-up bursts, back-to-back
  repeats, residual timers/listeners, or speech-scheduler duplication.

### ACC-OVERLAY-131 — Daypart weighting

- Contracts: `CTR-OVERLAY-034`.
- Method: inject local hours across the four buckets and boundaries into
  the pure selection matrices (both characters); verify weight-direction
  effects (late-night sleepy/rest bias, morning energy bias), no gated
  reactions, and no moralizing copy in any rendered line.
- Expected result: pure weight changes only; boundaries exact; copy clean.
- Failure condition: schedule-like gating, missing buckets, or lecture
  phrasing.

### ACC-OVERLAY-132 — Welcome-back and ritual bounds

- Contracts: `CTR-OVERLAY-035`, `CTR-OVERLAY-036`, `CTR-OVERLAY-010`.
- Method: preference-record matrix (absent/invalid/valid lastSeenAt;
  threshold boundary; quota/unavailable storage); mounted runs crossing
  the threshold exactly once; same-day returns producing no second
  welcome; first-completion-today and late-night once-per-day semantics
  across reload and local-day rollover; copy audit for guilt/streak/
  duration phrasing; Engine snapshot equality throughout.
- Expected result: one warm welcome, zero guilt, rituals once per day,
  silent safe fallbacks, growth untouched.
- Failure condition: repeat welcomes, guilt/duration copy, ritual spam,
  notification-channel use, or any growth mutation.

### ACC-OVERLAY-133 — Personality difference, sizes, and coexistence re-run

- Contracts: `CTR-OVERLAY-037`, `CTR-OVERLAY-020`, `CTR-OVERLAY-022`,
  `CTR-OVERLAY-003`, `CTR-OVERLAY-017`.
- Method: same-event matrices (completed, petting, ambient window, late
  night) across both characters proving observable presentation
  differences via pure-function outputs and rendered contact sheets at
  SMALL and LARGE; switch continuity (no cooldown reset); full composer/
  whale coexistence and bubble/label/footer clearance re-run with the new
  surfaces active.
- Expected result: two distinct, recognizable personalities; both sizes
  clean; coexistence and composer safety unchanged.
- Failure condition: identical presentations, a third subject, cooldown
  resets, occlusion regressions, or added focus targets.

### Coverage extension

CTR-029 → ACC-127; CTR-030 → ACC-127; CTR-031 → ACC-128; CTR-032 →
ACC-129; CTR-033 → ACC-130; CTR-034 → ACC-131; CTR-035 → ACC-132;
CTR-036 → ACC-132; CTR-037 → ACC-133. Every prior V6 contract/acceptance
pair remains binding wherever V7 did not replace its wording, evaluated
with both characters and both sizes wherever presentation is affected.

## 16. Authorization and reconciliation
V7 replacements CTR-015/018/019/021/028/010/022 supersede conflicting prior wording exactly as scoped in §15 Reconciliation; all other accepted obligations carry forward. The Owner's Goal「生命感」dispatch authorizes this bounded experience layer following the established docs-first independent review/acceptance/merge workflow. Previous production mandates are historical operation-specific records; actual deployment requires its own current operation record. No implementation begins before V7 acceptance is merged into its implementation base. PRODUCTION_APPLY_ALLOWED = NO for every round of this Goal.
...
NORMATIVE_TBD = NONE (band-valued constants are declared with recorded bounds; exact values are implementation conformance records)
