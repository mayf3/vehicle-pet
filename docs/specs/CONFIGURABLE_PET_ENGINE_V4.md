...
- Decision: L1–L5 mean the exact driver-seat, copilot safety-operator and rear-escort states in §11.1. Decorative hardware or color changes MUST NOT be represented as level meaning, stage title, milestone or keepsake meaning. Each level still binds its own existing stable subject asset ID. The blue-white car identity is retained. Existing art is illustrative, not evidence of operational occupancy; art/copy must not assert the opposite of the table. Any future art repair or humanoid mapping must represent these states. No new art is required for this copy correction; a visual mismatch must be recorded rather than reinterpreting the level.
