...
| `VEHICLE_PET_DEVELOPMENT_GOVERNANCE_ADOPTION_V2` | proposed | no — proposed governance upgrade successor on its adoption branch; predecessor stays accepted until atomic acceptance | invariant | none | `mayf3/vehicle-pet` | `VEHICLE_PET_DEVELOPMENT_GOVERNANCE_ADOPTION_V1` |
...
| `DSH_PET_OVERLAY_ADAPTER_V2` | accepted | yes once the exact accepted revision is reachable from `mayf3/vehicle-pet:main` (accepted candidate on its branch until merge) | implementation | contracts | `dsh-bundle-plugin`, `shell-overlay`, `compact-pet-surface`, `session-visual-reactions` | `DSH_PET_OVERLAY_ADAPTER_V1` |
