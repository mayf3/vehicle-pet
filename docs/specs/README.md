...
| `CONFIGURABLE_PET_ENGINE_V3` | superseded | yes once the exact accepted revision is reachable from `mayf3/vehicle-pet:main` (accepted candidate on its PR branch until merge) | implementation | contracts | `pet-engine`, `bundled-pet-packs`, `prototype-shell` | `CONFIGURABLE_PET_ENGINE_V2` |
