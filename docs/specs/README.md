...
| `CONFIGURABLE_PET_ENGINE_V2` | accepted | not yet; accepted candidate until the exact accepted revision is reachable from `mayf3/vehicle-pet:main` or a main-derived implementation base | implementation | contracts | `pet-engine`, `bundled-pet-packs`, `prototype-shell` | `CONFIGURABLE_PET_ENGINE_V1` |
